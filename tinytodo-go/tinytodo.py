...
def attempt_build():
    print('Attempting to build the tiny-todo-server via Go')
    proc = subprocess.run(['go', 'build', '-o', server_binary_path, './cmd/server'], check = True)
    ...
    def __init__(self, port):
        ...
        if not os.path.isfile(server_binary_path):
            try:
                ...
                print('Unable to build using Go!')
                return
        self.proc =  subprocess.Popen([server_binary_path, '-port', port])
...
server_binary_path = './tiny-todo-server'
