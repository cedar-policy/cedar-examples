...
server_binary_path = os.path.join(cargo_target_dir, "release", "tiny-todo-server")
...
def display_list(l):
    def inner(obj):
        ...
        lines = [title, id_line, owner_line, tasks_header] + [
            display_task(i + 1, task) for (i, task) in enumerate(list_of_tasks)
        ]
        return "\n".join(lines)
...
def display_task(index, task):
    return "%d: %s %s" % (
        index,
        "[ ]" if task["state"] == "Unchecked" else "[X]",
        task["name"],
    )
...
def create_task(user, list_id, name):
    url = "/api/task/create"
    data = {"uid": user.euid(), "list": List(list_id).euid(), "name": name}
...
def change_task_description(user, list_id, task_id, desc):
    ...
    task = find_task(user, lst, task_id)
    url = "/api/task/update"
    data = {"uid": user.euid(), "list": lst.euid(), "task": task["id"], "name": desc}
...
def share_list(user, list_id, share_with, read_only=True):
    ...
    return server.post(url, data), lambda _: "Shared list ID %s with %s as %s" % (
        l,
        share_with,
        "reader" if read_only else "editor",
    )
...
def unshare_list(user, list_id, unshare_with, read_only=True):
    ...
    return server.delete(
        url, data
    ), lambda _: "Unshared %s permissions on list ID %s with %s" % (
        "read" if read_only else "edit",
        l,
        unshare_with,
    )
