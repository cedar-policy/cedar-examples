...
def repo_entities(name, owner):
    ...
    return [
        entity(
            ...
            attrs={
                "readers": euid("RepoPermission", name+"_readers"),
                "triagers": euid("RepoPermission", name+"_triagers"),
                "writers": euid("RepoPermission", name+"_writers"),
                "maintainers": euid("RepoPermission", name+"_maintainers"),
                "admins": euid("RepoPermission", name+"_admins"),
                "owner": euid("Organization", owner)
            },
            parents = []
        ),
        entity(euid("RepoPermission", name+"_readers"), attrs={}, parents=[]),
        entity(euid("RepoPermission", name+"_triagers"), attrs={}, parents=[]),
        entity(euid("RepoPermission", name+"_writers"), attrs={}, parents=[]),
        entity(euid("RepoPermission", name+"_maintainers"), attrs={}, parents=[]),
        entity(euid("RepoPermission", name+"_admins"), attrs={}, parents=[]),
    ]
    ...
    for i in range(num_teams):
        ...
        for j in range(num_repos):
            if random.random() < chance_team_use_repo:
                parents.append(euid("RepoPermission", "repo_"+str(j)+random_role_str()))
...
def print_usage():
    print("Usage: github_entity_generator.py num_repos num_users num_teams num_orgs connection_probability [output_file_name]", file=sys.stderr)
